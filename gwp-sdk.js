...
(function(global) {
  ...
  class GopangWidget {
    ...
    constructor(handlers) {
      ...
      this._context  = this._decodeCtx('ctx');
    ...
    // 2026-09-22: 혼디 본체(engine.js _gwpLaunch)는 ctx를
    // btoa(unescape(encodeURIComponent(text)))로 base64 인코딩하고
    // ctx_enc=b64로 그 사실을 명시하는데, 이 SDK는 계속 plain
    // decodeURIComponent만 해서 한글 컨텍스트가 깨져서 들어오고 있었다.
    // ctx_enc=b64일 때만 base64 디코딩하고, 없으면(구버전 발신자 호환)
    // 기존처럼 plain URI 디코딩으로 동작한다.
    _decodeCtx(paramName) {
      var raw = this._params.get(paramName);
      if (!raw) return null;
      var enc = this._params.get(paramName + '_enc');
      if (enc === 'b64') {
        try {
          return decodeURIComponent(escape(atob(raw)));
        } catch (e) {
          console.warn('[GWP-SDK] ' + paramName + ' base64 디코딩 실패, plain으로 재시도:', e.message);
        }
      }
      try {
        return decodeURIComponent(raw);
      } catch (e) {
        console.warn('[GWP-SDK] ' + paramName + ' 디코딩 실패:', e.message);
        return raw;
      }
    }
